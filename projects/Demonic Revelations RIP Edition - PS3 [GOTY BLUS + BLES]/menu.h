void MainMenu()
{
	...
	add_title("<green>Red Trainer");
	DrawSubMenuOption("<SWAG>Self Options", Self_Options);
	DrawSubMenuOption("<VSKIP>Weapon Options", Weapon_Options);
	DrawSubMenuOption("<HOME>Teleport Options", Teleport_Options);
	DrawSubMenuOption("<ROCKSTAR>World Options", SubMenu4);
	DrawSubMenuOption("<PLAYER>Misc Options", Misc_Options);
	DrawSubMenuOption("<XP>Network Options", Network_Options);;
	DrawSubMenuOption("<MONEY>Spawner Options", SubMenu7);
	DrawSubMenuOption("<purple>Protection Options", ProtectIt);
}
void Sub_Menu1()
{
...
void SelfProtect()
{
	...
	DrawBoolOption("Antifreeze V3",freeze);
	...
	switch (getOption())
	{
	case 1: Option1: freeze = !freeze; if (freeze)DamagePro = 7; if (!freeze)DamagePro = 8; Protections(); break;
...
void Sub_Menu2()
{
	...
	DrawBoolOption("CaveExplosion", stealnpid);
	DrawBoolOption("ATRifle", changename);//force stop at 16 slots
	DrawBoolOption("t03BlockadeExplosion", DynamicId);
	DrawBoolOption("HolyWater", Precision);
	DrawBoolOption("ToxicZombie", Context);
	add_option("More Options");
	switch (getOption())
	{
	case 1: Option1: GiveActorAllWeapons(); break;
	case 2: Option2: AmmoInf(); break;
	case 3: Option3: CLEAR_LAST_ATTACK(self); stealnpid = !stealnpid; break;
	case 4: Option4: CLEAR_LAST_ATTACK(self); changename = !changename; break;
	case 5: Option5: CLEAR_LAST_ATTACK(self); Precision = 0; Context = 0; numbers = 0; DynamicId = !DynamicId; break;
	case 6: Option6: CLEAR_LAST_ATTACK(self); DynamicId = 0; Context = 0; numbers = 0; Precision = !Precision; break;
	case 7: Option7: CLEAR_LAST_ATTACK(self); DynamicId = 0; Precision = 0; numbers = 0; Context = !Context; break;
...
void WepMenu2()
{
	...
	DrawBoolOption("Killbot", session);//more so a Aim Killbot, rather than aimbot bleh w.e
	DrawBoolOption("Dynamite", TagInfoPrint);//npcs blow up like er, players look
	switch (getOption())
	{
	case 1: Option1: CLEAR_LAST_ATTACK(self); DynamicId = 0; Precision = 0; Context = 0; numbers = !numbers; if (numbers)print2("<green>Punch or Shoot a Human Ped To Become Them\nExplosive Rounds, Delete Gun, & Hell-Fire Gun Turned Off", 11000); break;
	...
	case 7: Option7: session = !session; if (session) { print2("<blue>Aim at Somebody Holding <lt> and You Will [Killbot Them], You Don't Even Have To Shoot", 5000); }; break;
	case 8: Option8: CLEAR_LAST_ATTACK(self); TagInfoPrint = !TagInfoPrint; break;
	}
}
void Sub_Menu3()
{
...
void Player_OpsSlot0()
{
	...
	DrawBoolOption("Freeze", bLocal563);
	add_option("Teleport To Me(N)");
	add_option("Teleport Into Vehicle");
	add_option("Spectate");
	DrawBoolOption("Explode", frontne);
	DrawBoolOption("TrainBoiler", ItemText);
	DrawBoolOption("Hell-Fire Explode", Value);
	add_option("More Options");
	switch (getOption())
	{
	case 1: Option1: bLocal563 = !bLocal563; if (bLocal563)FreezePlayerV2(NetSlotId); break;
	...
	case 5: Option5: erplode = 0; ItemText = 0; Value = 0; frontne = !frontne; break;
	case 6: Option6: erplode = 0; frontne = 0; Value = 0; ItemText = !ItemText; break;
	case 7: Option7: erplode = 0; frontne = 0; ItemText = 0; Value = !Value; break;
...
void Player_OpsSlot0P1()
{
	...
	switch (getOption())
	{
	case 1: Option1: ItemText = 0; Value = 0; frontne = 0; erplode = !erplode; break;
	...
	case 6: Option6: if (GET_LOCAL_SLOT() != NetSlotId)CopyPlayer(NetSlotId); break;
	case 7: Option7: changeSubmenu(NetPlayer1); break;
	case 8: Option8: FreezePlayerV1(NetSlotId); if (BHorse0 > 1294 || BHorse0 == -1)print2("<orange>You have no perms to freeze people with wagons!</orange>: <green>ON", 3000); FreezePlayerV1(NetSlotId); break;
    }
}
void Player_OpsSlot0P2()
{
	...
	case 2: Option2: UpdatePlayer = !UpdatePlayer; if (!UpdatePlayer)SET_ACTOR_STOP_UPDATE(GET_SLOT_ACTOR(NetSlotId), 0); break;
	}
}
void MenuSetup()
{
	if (MenuOpen)
	{
		...
		case Self_Options: Sub_Menu1(); break;
		case Weapon_Options: Sub_Menu2(); break;
		case Teleport_Options: Sub_Menu3(); break;
		case SubMenu4: Sub_Menu4(); break;
		case Misc_Options: Sub_Menu5(); break;
		case Network_Options: Sub_Menu6(); break;
