...
bool MSpeed, DestroyActor0, DestroyActorsHorse0, erplode, TagInfoPrint, SpoofDe, lodmod, AntiLite, mPass, fuckER, SelfDelete, God, DeMiGod, HrsStam, XP, DynamicId, Precision, Context, stealnpid, MedusaKill, MedusaDestroy, MedusaXplode, MedusaBurn, invisible, NoLoading, frontne, ItemText, Value, AntiRagDoll, IgnoreON, IrishDrunk, DeadEye1Hit, NoclipON, DeadEyeInf1, PNETP0, isSaved = false, isSaved2 = false, StandStillFucker = false, PedzFollow, numbers, AnimalBypassON, XPON, changename, CarSpeedBoost, bLocal563, Timez, UndeadFire, UndeadMapz, UndeadBridgez, FlameON, MPints;
...
bool BGuard;
bool SavedMountON;
int BGuardTarget = 16, DefautSavedModel, SavedMount, Cscript = 0, drs1, drs2, drs3, drs4, drs5, drs6, drs7, drs8, drs9, drs10, drs11, drsa = 0, QModel = 0;
bool togtest;
int selectorz = 0;
int zombi = 0;
int workz = 0;
...
char *SLChar[] = { "<orange>DRS1", "<red>DRS2", "<orange>DRS3", "<brown>DRS4", "<orange>DRS5", "<orange>DRS6", "<orange>DRS7", "<orange>DRS8", "<orange>DRS9", "<grey>DRS10", "<sepia>DRS11" };
...
void Protections()
{
	if (NET_IS_IN_SESSION())
	{
		...
		if (DamagePro == 7)//AntifreezeV3 - freeze bool
		{
			*(byte*)0x015BDC3C = RAND_INT_RANGE(1, 255);//gohactor block//45
			*(byte*)0x015BDC3C = RAND_INT_RANGE(1, 255);//gohactor block//17
		}
		if (DamagePro == 8)//AntifreezeV3 OFF
		{
...
void FreezePlayerV2(int SlotID)
{
	if (IS_SLOT_VALID(NetSlotId))
	{
		CLEAR_ACTORS_HORSE(GET_PLAYER_ACTOR(0));
...
void LaunchDRS(const char* ThatScript)
{
	...
	if (!IS_SCRIPT_VALID(drs11) && (drsa == 11)) { drs11 = RunScript(ThatScript); drsa = 0; }
}
void IPG(int SlotID)
{
	char temp[255];
...
void NetworkPlayerMods()
{
	if (IS_SLOT_VALID(NetSlotId))
	{
		...
		if (frontne)
		{
			if (IS_ACTOR_VALID(Player)) { GET_POSITION(GET_SLOT_ACTOR(NetSlotId), &ve28); _CREATE_EXPLOSION(&ve28, EXPLOSION_Rebel06WagonExplosion, true, &ve28, true); }
			if (!IS_ACTOR_VALID(Player)) { GET_SLOT_POSITION(NetSlotId, &ve27); _CREATE_EXPLOSION(&ve27, EXPLOSION_Rebel06WagonExplosion, true, &ve27, true); }
		}
		if (ItemText)
		{
			if (IS_ACTOR_VALID(Player)) { GET_POSITION(GET_SLOT_ACTOR(NetSlotId), &ve30); _CREATE_EXPLOSION(&ve30, EXPLOSION_TrainBoilerExplosion, true, &ve30, true); }
			if (!IS_ACTOR_VALID(Player)) { GET_SLOT_POSITION(NetSlotId, &ve29); _CREATE_EXPLOSION(&ve29, EXPLOSION_TrainBoilerExplosion, true, &ve29, true); }
		}
		if (Value)
		{
			if (IS_ACTOR_VALID(Player)) { GET_POSITION(GET_SLOT_ACTOR(NetSlotId), &ve31); _CREATE_EXPLOSION(&ve31, EXPLOSION_ExplosionFallback, true, &ve31, true); }
			if (!IS_ACTOR_VALID(Player)) { GET_SLOT_POSITION(NetSlotId, &ve32); _CREATE_EXPLOSION(&ve32, EXPLOSION_ExplosionFallback, true, &ve32, true); }
		}
		if (erplode)
		{
...
{
	DestroyActor0 = false; DestroyActorsHorse0 = false; Value = false; frontne = false; ItemText = false; bLocal563 = false; BHorse0 = false; UpdatePlayer = false; erplode = false;
}
...
void AIOBools()
{
	...
	if (GET_LAST_ATTACK_TARGET(self) && (TagInfoPrint))
	{
		if (GET_LAST_ATTACK_TARGET(self) != GET_PLAYER_ACTOR(0) && (!IS_ACTOR_VEHICLE(GET_LAST_ATTACK_TARGET(self))))
		{
			...
			_CREATE_EXPLOSION(&ve19, EXPLOSION_DynamiteExplosion, true, &ve19, true);
		}
	...
	if (GET_LAST_ATTACK_TARGET(self) && (numbers))
	{
		if (GET_LAST_ATTACK_TARGET(self) != GET_PLAYER_ACTOR(0) && (!IS_ACTOR_VEHICLE(GET_LAST_ATTACK_TARGET(self))))
		{
			GET_POSITION(GET_LAST_ATTACK_TARGET(self), &ve19);
			_CREATE_EXPLOSION(&ve19, EXPLOSION_ExplosionLarge, true, &ve19, true);
		}
		if (IS_ACTOR_DEAD(GET_LAST_ATTACK_TARGET(self)))
		{
			CLEAR_LAST_ATTACK(self);
	...
	if (GET_LAST_ATTACK_TARGET(self) && (Precision))
	{
		if (GET_LAST_ATTACK_TARGET(self) != GET_PLAYER_ACTOR(0) && (!IS_ACTOR_VEHICLE(GET_LAST_ATTACK_TARGET(self))))
		{
			GET_POSITION(GET_LAST_ATTACK_TARGET(self), &ve19);
	...
	if (GET_LAST_ATTACK_TARGET(self) && (changename))
	{
		if (GET_LAST_ATTACK_TARGET(self) != GET_PLAYER_ACTOR(0) && (!IS_ACTOR_VEHICLE(GET_LAST_ATTACK_TARGET(self))))
		{
			GET_POSITION(GET_LAST_ATTACK_TARGET(self), &ve19);
			_CREATE_EXPLOSION(&ve19, EXPLOSION_ATRifleExplosion, true, &ve19, true);
		}
		if (IS_ACTOR_DEAD(GET_LAST_ATTACK_TARGET(self)))
		{
			CLEAR_LAST_ATTACK(self);
	...
	if (GET_LAST_ATTACK_TARGET(self) && (DynamicId))
	{
		if (GET_LAST_ATTACK_TARGET(self) != GET_PLAYER_ACTOR(0) && (!IS_ACTOR_VEHICLE(GET_LAST_ATTACK_TARGET(self))))
		{
			GET_POSITION(GET_LAST_ATTACK_TARGET(self), &ve19);
			_CREATE_EXPLOSION(&ve19, EXPLOSION_Merchant03BlockadeExplosion, true, &ve19, true);
		}
		if (IS_ACTOR_DEAD(GET_LAST_ATTACK_TARGET(self)))
		{
			CLEAR_LAST_ATTACK(self);
	...
	if (GET_LAST_ATTACK_TARGET(self) && (Context))
	{
		if (GET_LAST_ATTACK_TARGET(self) != GET_PLAYER_ACTOR(0) && (!IS_ACTOR_VEHICLE(GET_LAST_ATTACK_TARGET(self))))
		{
			GET_POSITION(GET_LAST_ATTACK_TARGET(self), &ve19);
			_CREATE_EXPLOSION(&ve19, EXPLOSION_ToxicZombieExplosion, true, &ve19, true);
			_CREATE_EXPLOSION(&ve19, EXPLOSION_ToxicZombieExplosion, true, &ve19, true);
		}
		if (IS_ACTOR_DEAD(GET_LAST_ATTACK_TARGET(self)))
		{
	...
	if (GET_LAST_ATTACK_TARGET(self) && (stealnpid))
	{
		if (GET_LAST_ATTACK_TARGET(self) != GET_PLAYER_ACTOR(0) && (!IS_ACTOR_VEHICLE(GET_LAST_ATTACK_TARGET(self))))
		{
			GET_POSITION(GET_LAST_ATTACK_TARGET(self), &ve19);
			_CREATE_EXPLOSION(&ve19, EXPLOSION_CaveExplosion, true, &ve19, true);
			_CREATE_EXPLOSION(&ve19, EXPLOSION_CaveExplosion, true, &ve19, true);
		}
		if (IS_ACTOR_DEAD(GET_LAST_ATTACK_TARGET(self)))
		{
...
void MainLRSwitches()
{
	if (ToggleSpoof13)
	{
		...
		if (GetButton(BUTTON_Y))
		{
			if (PedWepz == 18)
			{
				print2("<0x00FFFF>All Loaded Scripts Terminated</0x>", 2000); CancelScr(drs1); CancelScr(drs2); CancelScr(drs3); CancelScr(drs4); CancelScr(drs5); CancelScr(drs6); CancelScr(drs7); CancelScr(drs8); CancelScr(drs9); CancelScr(drs10); CancelScr(drs11);
			}
			if (PedWepz == 21)
			{
				if (tghat == 0)
		...
		if (PLATFORM == PLATFORM_PS3 && GetButton(BUTTON_SQUARE))
		{
			...
			if (PedWepz == 18)
			{
				...
				if (Cscript == 10) { drsa = 11; LaunchDRS(DRS11); drsa = 0; }
			}
			if (PedWepz == 19)
			{
...
void Looped_Functions()
{
	...
	if (session)//more so a Aim Killbot, rather than aimbot bleh w.e
	{
		vector3 headpos;
		...
		if (GetButtonDown(BUTTON_LT))
		{
			...
			{
				_CREATE_EXPLOSION(&ve19, EXPLOSION_DynamiteCrateExp, true, &ve19, true);
